...
public class ClientDashboardController implements ClientPage {
    ...
    private void loadDashboard(String clientName) {
        List<ClientDashboardService.ProjectItem> projects = dashboardService.mockProjects();
        List<ClientDashboardService.OrderItem> orders = dashboardService.mockOrders();
        List<ClientDashboardService.ProjectItem> activeProjects = dashboardService.activeProjects(projects);
        List<ClientDashboardService.OrderItem> recentOrders = dashboardService.recentOrders(orders, 3);
        BigDecimal totalInvestment = dashboardService.totalInvestment(orders, projects);

        welcomeLabel.setText("Bem-vinda, " + clientName + "!");
        projectsCountLabel.setText(String.valueOf(activeProjects.size()));
